      ...
      <div className="relative grid gap-4 tablet:grid-cols-3 tablet:gap-6">
        ...
        {STEPS.map((step, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className="relative flex flex-col items-start gap-4 rounded-[16px] border border-slate-200 bg-white p-6 shadow-[0_12px_32px_rgba(38,87,166,0.06)]"
          >
            {/* 번호 + 아이콘 */}
            <div className="flex w-full items-center justify-between">
              <div
                className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${step.color} text-white shadow-md`}
              >
                {step.icon}
              </div>
              <span
                className={`text-4xl font-extrabold ${step.textColor} opacity-10 leading-none`}
              >
                {step.number}
              </span>
            </div>
            ...
            <div>
              <div className="mb-0.5 flex items-center gap-2">
                <span
                  className={`text-xs font-bold tracking-widest uppercase ${step.textColor}`}
                >
                  {step.label}
                </span>
              </div>
              <h3 className="text-base font-bold text-ftBlack">{step.title}</h3>
              <p className="mt-1.5 text-sm leading-relaxed text-ftGray break-keep">
                {step.desc}
              </p>
            </div>
          </motion.div>
