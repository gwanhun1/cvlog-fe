...
const ResumeFeatureSection = () => {
  return (
    <motion.section
      ...
      className="overflow-hidden rounded-[16px] border border-slate-200 bg-white px-5 py-10 shadow-[0_14px_40px_rgba(38,87,166,0.07)] tablet:px-10 tablet:py-12"
    >
        ...
        <div className="flex-1 flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            ...
            <h2 className="text-2xl font-extrabold leading-tight text-ftBlack tablet:text-3xl break-keep">
              개발자 이력서,
              <br />
              <span className="text-ftBlue">LOGME로 무료로 만들어요</span>
            </h2>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {FEATURES.map(f => (
              <div
                key={f.label}
                className="flex items-start gap-2.5 rounded-xl border border-ftBlue/10 bg-white px-3.5 py-3 shadow-sm"
              >
                <span className="mt-0.5 flex-shrink-0 text-ftBlue">
                  {f.icon}
                </span>
                <div>
                  <p className="text-xs font-bold text-ftBlack">{f.label}</p>
                  <p className="text-[11px] leading-tight text-ftGray">
                    {f.desc}
                  </p>
                </div>
              </div>
              ...
              {['경력', '프로젝트', '기술 스택'].map(label => (
                <div key={label} className="space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-ftBlue">
                      {label}
                    </span>
