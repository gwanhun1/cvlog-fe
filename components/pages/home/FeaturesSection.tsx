...
const FeaturesSection = ({ data }: Props) => {
  ...
  return (
    <motion.section
      ...
      className="w-full max-w-full overflow-hidden rounded-[16px] border border-slate-200 bg-white px-5 py-10 shadow-[0_14px_40px_rgba(38,87,166,0.07)] tablet:px-10 tablet:py-12"
    >
      {/* 섹션 헤더 */}
      ...
      <div className="flex flex-col gap-6 tablet:flex-row tablet:items-start tablet:gap-8">
        {/* 탭 목록 */}
        <div className="flex max-w-full gap-2 overflow-x-auto pb-1 tablet:w-52 tablet:flex-shrink-0 tablet:flex-col tablet:gap-2 tablet:pb-0">
          {data.map((item, i) => (
            <button
              key={item.id}
              type="button"
              onClick={() => handleTabClick(i)}
              className={`relative flex min-h-[44px] flex-shrink-0 items-center gap-2.5 rounded-[12px] px-4 py-3 text-left text-sm font-medium transition-all duration-200 focus-visible:ring-2 focus-visible:ring-ftBlue focus-visible:ring-offset-2 tablet:w-full ${
                i === active
              ...
              <div className="flex flex-col gap-4 tablet:w-64 tablet:flex-shrink-0 tablet:pt-2">
                <div>
                  <div className="mb-1 flex items-center gap-2">
                    ...
                    <h3 className="text-base font-bold text-ftBlack">
                      {current.title}
                    </h3>
                  </div>
                ...
                <div className="rounded-[12px] border border-ftBlue/15 bg-ftBlue/[0.04] p-3.5">
                  <div className="mb-1 text-xs font-semibold text-ftBlue">
                    활용 방법
                  </div>
