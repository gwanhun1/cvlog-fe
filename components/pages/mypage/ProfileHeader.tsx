...
> = {
  github: {
    ...
    icon: <FiGithub className="w-3.5 h-3.5 flex-shrink-0" aria-hidden="true" />,
  },
  google: {
    label: 'Google 계정',
    icon: <FcGoogle className="w-3.5 h-3.5 flex-shrink-0" aria-hidden="true" />,
  },
  kakao: {
    label: '카카오 계정',
    icon: (
      <RiKakaoTalkFill
        className="w-3.5 h-3.5 flex-shrink-0 text-[#FEE500]"
        aria-hidden="true"
      />
    ),
  },
  naver: {
    ...
    icon: (
      <SiNaver
        className="w-3.5 h-3.5 flex-shrink-0 text-[#03C75A]"
        aria-hidden="true"
      />
    ),
  },
        ...
        <div className="overflow-hidden relative w-16 h-16 rounded-full border-2 border-gray-100 shadow-sm flex-shrink-0">
          {profileImage ? (
            ...
            <div className="flex h-full w-full items-center justify-center rounded-full bg-slate-100">
              <FiUser aria-hidden="true" className="h-7 w-7 text-slate-500" />
            </div>
          )}
        </div>
