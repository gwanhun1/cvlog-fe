...
export default function PublicProfilePage({ username, page }: { username: string; page: number }) {
  ...
  return <main className="mx-auto max-w-4xl space-y-8 px-5 py-10">
    ...
    {resumes.length > 0 && <section><h2 className="mb-3 text-xl font-bold">공개 이력서</h2><div className="space-y-2">{resumes.map(r => <Link key={r.token} href={`/r/${r.token}`} className="block rounded-xl border border-slate-300 bg-white p-4 font-semibold text-blue-800">{r.title} ↗</Link>)}</div></section>}
