...
module.exports = {
  ...
  additionalPaths: async (config) => {
    try {
      ...
      // 필터링 후 공개 게시물만 선택
      const publicPosts = data.filter(post => post && post.public_status === true && post.id);
      console.log(`[Sitemap] Including ${publicPosts.length} public posts in sitemap`);
      ...
      return publicPosts.map(post => {
        ...
        const isImportantPost = post.title && (
